...
void EpiModel::response(void) {
  ...
  if (bTrigger && nTimer==nTriggerTime) {
	...
	if (t.id<2) cout << "Working from home in tract " << t.id << " on day " << (nTimer/2) << " for " << nWorkFromHomeDuration << " days" << endl;
      }
	...
	if (t.id<2) cout << "Liberal leave in tract " << t.id << " on day " << (nTimer/2) << " for " << nLiberalLeaveDuration << " days" << endl;
      }
 ...
 */
void EpiModel::summary(int current_day) {
  ostringstream oss;
  ofstream &outfile = *sumfile;
  //printf("sumday %d %d\n",current_day,nRunLength);
  if (current_day==0)
  {
    ...
    outfile << "DailySummary:" << endl;
  }

  outfile << "  - Day: " << current_day << endl;
  if (current_day==nRunLength-1) // last day produce more output-A
  {
    ...
    if (nSeedInfectedTractFIPS>0 || nSeedInfectedCountyFIPS>0 || nSeedInfectedStateFIPS>0)
      ...
      outfile << "    Seeded infected people: " << nSeedInfectedNumber<< endl;
    ...
    if (nSchoolClosurePolicy==0)
      outfile << "    School closure policy: null" << endl;
  ...
  if (current_day==nRunLength-1) // last day produce more output-B
  {
  ...
  if (current_day==nRunLength-1) // last day produce more output-C
  { 
...
void EpiModel::prepH5(EpiModelParameters &parms) {
  int numTract=tractvec.size();
  int numTAG=TAG; // total_age_groups 
  int numDay=nRunLength; // how many simulated days
  printf("prepH5: numDay=%d, numTAG=%d, numTract=%d\n",numDay,numTAG,numTract);

  epiH5= new EpiData2hd5(numTract,numTAG,numDay);
  epiH5->addLabel("R0",parms.getR0(),0,5);
  epiH5->addLabel("TriggerDay",parms.getTriggerDay(),1,365);
  epiH5->addLabel("SchoolClosureDays",parms.getSchoolClosureDays(),1,365);
  epiH5->addLabel("LiberalLeaveCompliance",parms.getLiberalLeaveCompliance());
  epiH5->addLabel("QuarantineCompliance", parms.getQuarantineCompliance());
  epiH5->addLabel("WorkFromHomeCompliance", parms.getWorkFromHomeCompliance());
  epiH5->addLabel("WorkFromHomeDuration", parms.getWorkFromHomeDuration(),1,365);
  epiH5->addLabel("VoluntaryIsolationCompliance",parms.getVoluntaryIsolationCompliance() );
  epiH5->addLabel("AscertainedIsolationCompliance",parms.getAscertainedIsolationCompliance() );

}

void EpiModel::updateTractHistory(int iDay){
  // count number of infecteds by tract
  //printf("\niday=%d\n",iDay);
  int iTr=-1;
  for (vector< Tract >::iterator it = tractvec.begin();  it != tractvec.end(); it++) {
    Tract& t = *it; iTr++;
    for (unsigned int i=t.nFirstCommunity; i<t.nLastCommunity; i++) {
      for (int j=0; j<TAG; j++) {
        epiH5->addTractItem(iTr,j,iDay,commvec[i].nEverSymptomatic[j]);
      }
    }
  }
}

void EpiModel::saveH5(string coreName) {
  //printf("EpiModel::saveH5()\n");
  epiH5->doDayDiff();// convert integrals to day-differences
  //epiH5->dumpLabels();
  //epiH5->dumpNonZeroTract(); // dump before saving to H5
  epiH5->writeH5(coreName);
}


const EpiData2hd5 *EpiModel::getEpiDataInfo() {
  return epiH5->getEpiDataInfo();
}
